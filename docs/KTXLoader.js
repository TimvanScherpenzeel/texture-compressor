...
var KhronosTextureContainer = (function() {
	...
	function KhronosTextureContainer(arrayBuffer, facesExpected) {
		...
		this.glBaseInternalFormat = oppositeEndianess
			? this.switchEndainness(header[5])
			: header[5]; // specify GL_RGB, GL_RGBA, GL_ALPHA, etc (un-compressed only)
		...
		this.numberOfArrayElements = oppositeEndianess
			? this.switchEndainness(header[9])
		...
		this.numberOfMipmapLevels = oppositeEndianess
			? this.switchEndainness(header[11])
			: header[11]; // number of levels; disregard possibility of 0 for compressed textures
		this.bytesOfKeyValueData = oppositeEndianess
			? this.switchEndainness(header[12])
